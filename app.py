...
NAV = ["Featured trades", "Trade simulator", "Player rankings", "Methodology", "Articles", "Contact"]
...
if clicked:
    st.session_state.tab = clicked
    st.session_state.article = None   # leaving the reader view
if "article" not in st.session_state: st.session_state.article = None
for i, n in enumerate(NAV):
...
# ---------------------------------------------------------------- ARTICLES
    ...
    with M:
        if st.session_state.get("article"):
            articles.reader(st.session_state.article)
        else:
            articles.index()
            show_site_footer('contact_articles')
