...
# ---------------------------------------------------------------- OPPORTUNITIES
    ...
    if not len(H):
        ...
        st.markdown('<div style="font-family:Archivo,sans-serif;font-weight:700;font-size:12px;'
                    ...
                    unsafe_allow_html=True)
        elig_key = f"_elig_{team}"
        if elig_key not in st.session_state:
            box0 = splash("Checking which holes have a trade clearing fair value", vh=30, size=22)
            elig = []
            for slot in H.slot.tolist():
                PKc = OP.package(team, slot=slot, top=8, C=B)
                Pc = PKc.get('packages')
                if Pc is not None and len(Pc) and Pc.both_gain.any():
                    elig.append(slot)
            st.session_state[elig_key] = elig
            box0.empty()
        ...
        if not elig_slots:
            ...
            pick_slot = c1.selectbox("Which hole", elig_slots, key="oppslot")
