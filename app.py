...
NAV = ["Featured trades", "Team opportunities", "Trade simulator", "Player rankings", "Methodology", "Contact"]
...
# ---------------------------------------------------------------- OPPORTUNITIES
    ...
    B = engine.boot()
    st.markdown('<div class="sechead">Team Opportunities</div><p>Where a squad is short before '
                'anyone has been released, and what the auction is likely to do about it. Every slot '
                'is judged against the par for that position, and gaps are ranked by what they cost '
                'the side \u2014 a shortfall at the top of the order matters more than the same '
                'shortfall at seven, because the opener faces roughly three times the balls.</p>',
                unsafe_allow_html=True)
    team = st.selectbox("Team", B['teams'], index=B['teams'].index('RCB'))
    R = OP.report(team, top=3, B=B)
    S = R['summary']

    m = st.columns(5)
    m[0].metric("Best XII now", f"{S['xii_value']:.2f}")
    m[1].metric("After the auction", f"{S['xii_after_auction']:.2f}", f"{S['recovery']:+.2f}")
    m[2].metric("Purse", f"\u20b9{S['purse']:.2f} cr")
    ...
    st.markdown('<div style="font-family:Archivo,sans-serif;font-weight:700;font-size:12px;'
                'letter-spacing:.7px;text-transform:uppercase;color:#6B7A6F;margin:22px 0 6px">'
                'The side that actually played, 2026</div>', unsafe_allow_html=True)
    AP = OP.as_played(team, C=B)
    ap = AP.copy()
    ap['still_at_club'] = ap.still_at_club.map({True: "", False: "released"})
    ap['overseas'] = ap.overseas.map({1: "Overseas", 0: "Indian"})
    ap = ap[['slot', 'need', 'player', 'basis', 'matches', 'pWAR', 'war_matches',
             'overseas', 'still_at_club']]
    ap.columns = ['Slot', 'Eligible from', 'Player', 'Filled by', 'Matches there',
                  'Projected WAR', 'WAR \u00d7 matches', 'Nationality', 'Status']
    st.dataframe(ap, hide_index=True, use_container_width=True)
    st.caption("Built slot by slot from what actually happened. Batting position is derived from "
               "the order men reached the crease, so only players who genuinely opened can fill "
               "1 and 2, only a number three can fill 3, and so on. Slots 9\u201312 go to bowlers "
               "by the phase each covered \u2014 measured against how much of the innings that "
               "phase is, so a death bowler is a man who bowls the death more than the four overs "
               "on offer would predict, not merely someone who bowled there once. Within a group, "
               "candidates rank on WAR \u00d7 matches. A batter who never reached the crease "
               "leaves no trace, which is correct: he did not occupy the position that day.")

    st.markdown('<div style="font-family:Archivo,sans-serif;font-weight:700;font-size:12px;'
                'letter-spacing:.7px;text-transform:uppercase;color:#6B7A6F;margin:26px 0 6px">'
                'The best XII available, against par</div>', unsafe_allow_html=True)
    p = R['positions'].copy()
    p['overseas'] = p.overseas.map({True: "Overseas", False: "Indian"})
    p['wk'] = p.wk.map({True: "Keeper", False: ""})
    p = p[['bat_slot', 'group', 'judged_on', 'basis', 'player', 'pWAR', 'par', 'gap',
           'weight', 'gap_value', 'status', 'overseas', 'wk']]
    p.columns = ['Slot', 'Position', 'Judged on', 'Against', 'Player', 'Projected WAR',
                 'Par', 'Shortfall', 'Slot weight', 'Weighted shortfall', 'Verdict',
                 'Nationality', 'Role']
    st.dataframe(p, hide_index=True, use_container_width=True)
    st.caption("Slots 1\u20137 are judged on batting against the par for that batting group. "
               "Slots 8\u201312 are bowlers, so they are judged on bowling instead \u2014 against "
               "a par blended from bench_bowl.csv by the phases each man actually bowled in 2026, "
               "shown in the 'Against' column. A powerplay bowler and a death bowler are not held "
               "to the same standard.")

    H = R['holes']
    if not len(H):
        st.success(f"No slot in {team}'s best XII sits more than "
                   f"{OP.HOLE_TOL} WAR below par. There is nothing obvious to fix.")
    else:
        st.markdown('<div style="font-family:Archivo,sans-serif;font-weight:700;font-size:12px;'
                    'letter-spacing:.7px;text-transform:uppercase;color:#6B7A6F;margin:26px 0 6px">'
        ...
        for c in ['p_fill', 'p_at_par']:
            h[c] = (h[c] * 100).round(0).astype('Int64').astype(str) + '%'
        h = h[['slot', 'unit', 'basis', 'occupant', 'pWAR', 'par', 'gap', 'gap_value',
               'pool_n', 'pool_at_par', 'pool_affordable', 'best_available',
               'p_fill', 'p_at_par']]
        h.columns = ['Position', 'Judged on', 'Against', 'Current occupant', 'His WAR',
                     'Par', 'Shortfall', 'Weighted shortfall', 'Players in the pool',
                     'Of those, at par', 'At par and affordable', 'Best available',
                     'Fills at all', 'Fills at par']
        st.dataframe(h, hide_index=True, use_container_width=True)
        st.caption("Bowling holes have no fill probability because engine._auction only raises "
                   "batting requirements \u2014 blank rather than invented. Pool counts are the 2027 auction pool \u2014 every player on a release list, "
                   "everyone unsold, plus a draw of new entrants. The two right-hand columns come "
                   "from the same auction simulation the trade simulator uses; read them alongside "
                   "the pool counts rather than on their own.")

    if len(H):
        st.markdown('<div style="font-family:Archivo,sans-serif;font-weight:700;font-size:12px;'
                    'letter-spacing:.7px;text-transform:uppercase;color:#6B7A6F;margin:30px 0 6px">'
                    'Fix it by trade</div>'
                    '<p style="margin-top:0">Every player on the other nine squads is tested against '
                    'this hole: the XII is re-solved with him in it, re-solved again at his own club '
                    'without him, and a return is searched for on your squad that covers what they '
                    'lose. Only deals that leave both clubs ahead are worth calling trades.</p>',
                    unsafe_allow_html=True)
        c1, c2 = st.columns([2, 1])
        pick_slot = c1.selectbox("Which hole", H.slot.tolist(), key="oppslot")
        go = c2.button("Find trades", type="primary", use_container_width=True)
        if go:
            box = splash("Re-solving both XIs for every candidate on the other nine squads", vh=30, size=22)
            PK = OP.package(team, slot=pick_slot, top=4, C=B)
            box.empty()
            P = PK.get('packages')
            if P is None or not len(P):
                st.info("No player on another squad both closes this hole and can be paid for "
                        "out of the purse available.")
            else:
                good = P[P.both_gain]
                if not len(good):
                    st.warning("Candidates exist, but none leaves the selling club whole. "
                               "The deals below need cash or a third player to work.")
                show = (good if len(good) else P).copy()
                t = show[['gets', 'from_team', 'gives', 'cash', 'buy_gain', 'buy_net',
                          'sell_net', 'saving']].copy()
                t.columns = ['You get', 'From', 'You send back', 'Cash out (\u20b9cr)',
                             'Raw XII gain', 'Net XII gain', 'Their net', 'Cheaper than auction (\u20b9cr)']
                st.dataframe(t.round(2), hide_index=True, use_container_width=True)
                for r in show.itertuples():
                    st.markdown(
                        f'<div style="border-left:3px solid #C9C2A8;padding:8px 0 8px 14px;margin:10px 0">'
                        f'<div style="font-family:Archivo,sans-serif;font-weight:700;font-size:14px;'
                        f'color:#2A3A30">{r.gets}'
                        + (f' \u2194 {r.gives}' if r.gives else '') +
                        f'</div><div style="font-family:\'Source Serif 4\',Georgia,serif;font-size:14px;'
                        f'color:#4A5A50;line-height:1.6;margin-top:3px">{r.why}</div></div>',
                        unsafe_allow_html=True)
                st.caption("Raw XII gain is the incoming player alone. Net XII gain is after "
                           "subtracting what the return costs you. 'Cheaper than auction' is his "
                           "market price under the fitted price model minus the salary you take on "
                           "\u2014 the saving from going by trade rather than bidding.")
