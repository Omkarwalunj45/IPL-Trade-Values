...
# ---------------------------------------------------------------- OPPORTUNITIES
    ...
    st.markdown('<div style="font-family:Archivo,sans-serif;font-weight:700;font-size:12px;'
                ...
                'Every position against par</div>', unsafe_allow_html=True)
    p = R['positions'].copy()
    p['overseas'] = p.overseas.map({True: "Overseas", False: "Indian"})
    p['wk'] = p.wk.map({True: "Keeper", False: ""})
    p = p[['bat_slot', 'group', 'player', 'pWAR', 'par', 'gap', 'weight', 'gap_value',
           'status', 'overseas', 'wk']]
    p.columns = ['Bat slot', 'Position', 'Player', 'Projected WAR', 'Par for the slot',
                 'Shortfall', 'Slot weight', 'Weighted shortfall', 'Verdict',
                 'Nationality', 'Role']
    st.dataframe(p, hide_index=True, use_container_width=True)
    st.caption("Slots 8\u201312 carry no batting requirement \u2014 8 is a bowling allrounder and "
               "9\u201312 are bowlers, so par is blank rather than missing. Slot weight is exposure "
               "\u00d7 importance, the same weighting the XII optimiser maximises.")
    ...
    if not len(H):
        ...
        h = h[['slot', 'occupant', 'pWAR', 'par', 'gap', 'gap_value', 'pool_n',
               'pool_at_par', 'pool_affordable', 'best_available', 'p_fill', 'p_at_par']]
        h.columns = ['Position', 'Current occupant', 'His WAR', 'Par', 'Shortfall',
                     'Weighted shortfall', 'Players in the pool', 'Of those, at par',
                     'At par and affordable', 'Best available', 'Fills at all',
                     'Fills at par']
        st.dataframe(h, hide_index=True, use_container_width=True)
        st.caption("Pool counts are the 2027 auction pool \u2014 every player on a release list, "
