...
# Stardom is now handled by one hard list and nothing else.  A franchise icon is not
# a valuation problem to be priced, he is simply not for sale: Mumbai do not trade
# Rohit or Bumrah, Chennai do not trade Dhoni.  Everyone else is judged on what the
# model can actually measure, without a premium layered on top.
UNTRADEABLE = {
    'Virat Kohli', 'Jasprit Bumrah', 'Shubman Gill', 'Suryakumar Yadav',
    'Rohit Sharma', 'Sunil Narine', 'MS Dhoni',
}

# The package going back must sit in a band around the man arriving -- not so big that
# you gave up more than you got, not so small that a blocked player leaves for a
# token.  Measured on plain pWAR now that the premium is gone.
RETURN_FLOOR = 0.60
PKG_TOL = 0.15

# Below this the improvement is inside the projection's own noise, and dividing a
# rounding error by a base-price salary produces a spectacular, meaningless
...
def why(r, slot, occupant):
    """One line explaining the row, built from its own numbers."""
    bits = [f"Slots straight in at {str(slot).lower()}, {r.d_buy:+.2f} on the best XII "
            f"over {occupant}."]
...
def package(team, slot=None, top=3, C=None, verify=False):
    ...
    for r in d.itertuples():
        ...
        # is not a trade
        # No symmetry cap: a deal where the seller gains far more is exactly the
        # surplus-for-need swap that makes up most real trades, not a red flag.
...
def best_return(buyer, seller, need, max_loss, in_salary, C=None, max_players=2,
    ...
    def ok_player(p):
        ...
        if in_pwar is not None and w >= in_pwar:
            return False
        return True
    ...
    def ok_pkg(players):
        ...
        tot = sum(float(meta.loc[p, 'pWAR'] or 0) for p in players if p in meta.index)
        want = float(in_pwar or 0)
