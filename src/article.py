...
('t', "There are four problems to solve before a trade can be priced. **Valuation:** What’s a player’s intrinsic worth, isolated from the team context. **Projection:** What is he likely to be worth next season and the one after that. This is much harder in a league as short and volatile as the IPL. **Team Fit:** A player is never worth the same to every side. A team is basically twelve slots (once you factor in the impact player) and a set of constraints. **Replacement:** When you trade a player away you go into an auction with a limited purse, competing against nine other teams for a pool of players that nobody knows during the time of trades."),
...
('t', "The first step is putting players into the same unit. I use **Runs Above Replacement**, converted into wins, to measure a player's contribution. The model starts at the ball level and estimates how much value was created relative to what would normally be expected from that match state."),
('h', 'Run Expectancy'),
...
('t', "So now we've got something substantial- there's a value, a price, and a probability of recovery. The last thing left is to actually run the season and see what changes wrt States S1 and S2 as they're 2 different states of the same team based on the decision they took in the trade. S1: No trade so the player(s) stays. The team goes into the release process with its original squad, builds its purse, enters the auction and fills whatever vacancies it has. S2: Trade. The trade happens first which in turn changes the squad, the salaries and the vacancies. The release decisions are then applied to that new state, the purse changes accordingly, and the team enters the auction with a different set of needs."),
('h', 'Season Simulation'),
