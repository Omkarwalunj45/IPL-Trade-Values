...
def build_players(verbose=True):
    ...
    bbb['bat']=fix(bbb.bat); bbb['bowl']=fix(bbb.bowl)
    ...
    w=pd.read_parquet(WAR_PATH); w['player']=fix(w.player)
...
# Natural batting slots the ball-by-ball data gets wrong on thin IPL evidence.
# These are corrected by hand from how the player is actually used.
NAT_SLOT_FIX = {'Lhuan-dre Pretorius': 1}
...
def build_players_pwar(verbose=False):
    ...
    W=pd.read_csv(PWAR_PATH); W['player']=fix(W.player)
    ...
    for _p, _s in NAT_SLOT_FIX.items():
        M.loc[M.player == _p, 'nat_slot'] = _s
    if verbose:
...
def squad_pwar(P, team, add=(), drop=(), fillers=True):
    """Squad after a trade.  A club that loses its only opener does not cease to
       exist: it goes to the auction with a hole it must fill.  So replacement-level
       cover (pWAR 0) is always available for every slot, which keeps the XII legal
       and turns the gap into a vacancy the auction then has to solve."""
    ...
    rows = []
    for s_ in range(1, 9):
        rows.append(dict(player='Replacement bat %d' % s_, nat_slot=s_, pWAR=0.0,
            share_bat=1.0, share_bowl=0.0, bat_rate=0.0, bowl_rate=0.0, phases=set(),
            can_bowl=False, overseas=0, is_wk=(s_ in (4, 5)), role='Batter',
            ipl_balls_faced=0, ipl_balls_bowled=0, salary=0.30, team=team))
    rows.append(dict(player='Replacement bowler', nat_slot=9, pWAR=0.0,
        share_bat=0.0, share_bowl=1.0, bat_rate=0.0, bowl_rate=0.0,
        phases={'Powerplay','Middle','Death'}, can_bowl=True, overseas=0, is_wk=False,
        role='Bowler', ipl_balls_faced=0, ipl_balls_bowled=0, salary=0.30, team=team))
    f = pd.DataFrame(rows)
    for c in sq.columns:
        if c not in f.columns: f[c] = sq[c].iloc[0] if len(sq) else 0
    return pd.concat([sq, f[sq.columns]], ignore_index=True)
