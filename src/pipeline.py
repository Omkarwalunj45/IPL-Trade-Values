...
def load_state():
    ...
    ros['player']=ros.player.str.strip().replace(ALIAS)
    ...
    rel['player']=rel.player.str.strip().replace(ALIAS)
    ...
    pool['player']=pool.player.str.strip().replace(ALIAS)
...
def projections(squad, pool):
    ...
    _a['_p']=_a.playerName.str.strip().replace(ALIAS)
    def _cr2(x):
    ...
    a=a[a.year==2026].copy(); a['player']=a.playerName.str.strip().replace(ALIAS)
