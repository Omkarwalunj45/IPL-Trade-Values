...
def vacancies(P, team, bench_bat, bench_bowl, add=(), drop=()):
    ...
    for r in xii.itertuples():
        # slots 8-12 are bowlers and a bowling allrounder.  They are picked to bowl,
        # so holding them to a batting par invents a hole nobody would ever fix by
        # buying a batter.  engine._auction already dropped these (its EX table has
        # no Tail key); not creating them in the first place keeps the two agreed.
        g=grp.get(int(r.bat_slot))
